        ...
        pollSosAlerts() {
            fetch('{{ route('peace.sos.alerts') }}')
                ...
                .then(data => {
                    ...
                    // Sound siren whenever brand new alert arrives
                    if (this.hasInitialPollRun && brandNewAlerts.length > 0) {
                    ...
                    @if(!empty($customTemplate))
                        <div style="margin-bottom:16px;background:#f0fdf4;border:1.5px solid #86efac;border-radius:12px;padding:14px 18px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;box-shadow:0 2px 10px rgba(22,163,74,0.06);">
                            ...
                            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                                ...
                                <form action="{{ route('department.reports.delete_template') }}" method="POST" style="margin:0;" onsubmit="return confirm('Revert to the standard system template matrix for Peace & Order?');">
                                    @csrf
                                    @method('DELETE')
                                    <input type="hidden" name="department" value="Peace & Order">
